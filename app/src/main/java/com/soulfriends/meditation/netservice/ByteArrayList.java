...
public class ByteArrayList {
     ...
     */
    public void ensurePreCapacity(int minCapacity) {
        if (minCapacity < 0) {
        ...
        setPreSize((int)Math.ceil(newSize));
    }
    ...
    void setPreSize(int size) {
        ...
        System.arraycopy(mStorage, 0, newStorage, mMaxBytes-mCurBytes, mCurBytes);
        mStorage = newStorage;
    }
    ...
    public boolean addPreAll(byte[] src) {
        if (src == null) {
            throw new NullPointerException();
        }
        return addPreAll(src, 0, src.length);
    }
    /**
    ...
    public boolean addPreAll(byte[] src, int srcOffset, int length) {
        if (src == null) {
            throw new NullPointerException();
        }
        ensurePreCapacity(mCurBytes + length);
        System.arraycopy( src, srcOffset, mStorage, 0, length);
        mCurBytes += length;
        return true;
    }
    /**
